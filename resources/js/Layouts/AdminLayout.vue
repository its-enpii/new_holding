...
<template>
    <div class="min-h-screen bg-surface font-sans text-on-surface">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-outline-variant/50 bg-gradient-to-b from-surface-container-lowest via-surface to-surface-container-low py-6 transition-transform duration-300 lg:translate-x-0" :class="mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'">
            <div class="mb-8 px-6">
            ...
            <div class="mx-4 flex items-center gap-3 rounded-lg border-t border-primary-container bg-primary-container/50 p-3 pt-4">
                <span class="grid size-10 shrink-0 place-items-center rounded-full bg-primary-fixed text-sm font-semibold text-primary">{{ user?.name?.charAt(0).toUpperCase() || 'A' }}</span>
        ...
        <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-outline-variant bg-gradient-to-r from-surface-container-lowest/90 to-surface-container-low/90 px-4 backdrop-blur-sm lg:ml-64 lg:px-6">
